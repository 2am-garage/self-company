...
| `UserPromptSubmit` | — | `hook_memory_inject.py` | **30s** | Ask-time memory injection: ranks L2/high-rc L1 by a **fast stdlib** scorer and injects top-k as `additionalContext`. Relevance-gated (injects nothing if nothing scores), token-capped, never blocks. **No fastembed cold-start on this path** (30s cap). Also injects a SEPARATE, always-on, ungated "core identity" block (§8.2, capped) of explicitly opted-in/well-reinforced L2 facts on every turn, concatenated after the relevance-gated block — the two never merge and the relevance gate is unaffected. |
