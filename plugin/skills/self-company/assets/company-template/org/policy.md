...
| `SELF_COMPANY_INJECT_RAG_MIN_SCORE` | **0.35** | Cosine relevance floor for semantic injection (ask-time hook + per-employee/shared recall). Retuned from 0.30 (Phase 24, data-driven sweep on the real corpus post model-swap): off-topic top-1 scores topped out ~0.31, the lowest true on-topic top-1 was ~0.42 — 0.35 sits in that gap. | ✓ (env override) |
