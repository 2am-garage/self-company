...
| `SELF_COMPANY_INJECT_RERANK_MIN_SCORE` | **-2.75** | Cross-encoder reranker cutoff (Phase 24 Item 5) — the FINAL relevance gate for semantic injection when the reranker backend is present. A hit injects iff it clears BOTH the cosine floor AND this cutoff. Data-driven: the midpoint of the innocent-off-topic ("gym workout" ~-3.0) ↔ lowest-cosine-passing-on-topic (~-2.5) gap. Reranker absent → gate skipped, cosine floor alone decides. See `references/rag.md §13`. | ✓ (env override) |
