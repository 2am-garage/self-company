...
| `SELF_COMPANY_INJECT_RAG_MIN_SCORE` | **0.40** | Cosine relevance floor for semantic injection (ask-time hook + per-employee/shared recall). Retuned across Phase 24 (0.30 → 0.35 → 0.40, data-driven). 0.40 is the HIGHEST floor that still keeps EVERY on-topic diagnostic hit: the lowest true-positive top-1 on the real corpus is `merge-gate` at 0.419, so the floor sits just below it. It cannot go higher without dropping a real hit (see `references/rag.md §13` for the reranker escalation on the one innocent-off-topic prompt that scores in the 0.417–0.419 band). | ✓ (env override) |
