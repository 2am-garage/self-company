...
| `RAG_MODEL` | **`sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2`** | fastembed embedding model, local CPU, runs fully offline; 384-dim, MULTILINGUAL (Phase 24 — fixes cross-lingual retrieval, e.g. the Chairman's Traditional-Chinese prompts, which the prior English-only model scored no better than random). No API embeddings, no daemon — privacy is a hard rule; memory content never leaves the machine. (Legacy Ollama `nomic-embed-text`/768-dim, then `BAAI/bge-small-en-v1.5`/384-dim English-only, are both superseded. Model+dim is a single table in `rag_embed.py`; a future swap can never miss a dim assertion. The index is model-stamped — see `references/rag.md §1/§13`.) | ✓ |
| `RAG_INDEX_PATH` | **`.company/memory/index`** | LanceDB vector store location; matches reserved folder in design §2. Gitignored and private; index is rebuildable from markdown, not a source of truth. Carries a native FTS (BM25) index for hybrid retrieval (Phase 24 Item 4). | ✓ |
| `SELF_COMPANY_INJECT_RAG_MIN_SCORE` | **0.40** | Cosine relevance floor for semantic injection (ask-time hook + per-employee/shared recall). Retuned across Phase 24 (0.30 → 0.35 → 0.40, data-driven). 0.40 is the HIGHEST floor that still keeps EVERY on-topic diagnostic hit (lowest true-positive top-1 = `merge-gate` 0.419). As of Item 5 it is a cheap PRE-filter ahead of the reranker gate. | ✓ (env override) |
| `SELF_COMPANY_INJECT_RERANK_MIN_SCORE` | **-2.75** | Cross-encoder reranker cutoff (Phase 24 Item 5) — the FINAL relevance gate for semantic injection when the reranker backend is present. A hit injects iff it clears BOTH the cosine floor AND this cutoff. Data-driven **best-separation** point (sits just below the on-topic cluster) — NOT a clean gap: off-topic/on-topic rerank scores interleave near the boundary, so this is a best-effort precision cutoff, not a perfect separator (see `references/rag.md` "Known limits"). Reranker absent → gate skipped, cosine floor alone decides. | ✓ (env override) |
