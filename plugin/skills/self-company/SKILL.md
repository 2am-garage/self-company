...
- `forget_memory.py` — Chairman-driven HARD FORGET: `--forget <id>` tombstones one EXPLICIT memory id immediately (any tier, overriding L2's never-decay rule), audits it (`op: forget`), and single-id-deletes it from the live RAG index; requires `--yes`/confirmation, refuses a blessed charter id without `--force-charter` (memory-tiers.md §10).
