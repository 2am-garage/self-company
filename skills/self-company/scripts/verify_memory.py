...
# Bucket 2 (Phase 14): the shared sibling modules (charter_ids, tombstone,
# frontmatter) live in THIS directory. Put it on sys.path FIRST so the hard
# imports below resolve under every entry point — direct run, cron, a hook, or an
# import by another module / the test harness (mirrors schedule_validator.py).
# They always ship together, so the imports never fail.
...
# Phase 4 Item 1 — verify_memory, entropy, and decay all import it (hard import;
# the sibling always ships here, so it cannot fail).
...
# the ONE shared module (frontmatter.py, same dir) so the legacy per-scanner
# parsers can't drift — the `.strip() == '---'` delimiter is the single source.
# verify keeps its own `(None, text)` no-frontmatter sentinel and all downstream
